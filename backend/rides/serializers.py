...
class TripSerializer(serializers.ModelSerializer):
    ...
    def get_fare(self, object):
        base_fare = 1000
        driver = object.driver
        active_trips_of_driver = Trip.objects.filter(
            driver=driver, status__in=["PENDING", "ONGOING"]
        ).count()
        # (donde active_trips_of_driver cuenta trips en PENDING u ONGOING para ese driver)
        surge_multiplier = 1 + (
            active_trips_of_driver / 10
        )  
